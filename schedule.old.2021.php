...
module::getModuleClass('breadcrumbs')->addElement('/index.php?component=system&do=generateXLSX&template=schedule&type=xlsx&mode=default&id=15&filename='.urlencode('График производства '.date('Y-m-d')), '<i class="fa fa-file-excel"></i> excel');

$processingOrders = engine::DB()->getAll('SELECT * FROM @[prefix]moysklad_processingOrders WHERE ready < quantity AND applicable=1 ORDER BY moment ASC');
...
$specificationsPP =engine::DB()->getAll('
SELECT @[prefix]pp.id as processingPlanId_moySklad, @[prefix]specification.id as specificationId, @[prefix]specification.modificationId as modificationId, @[prefix]specification.revision as specificationRevision, @[prefix]modifications.name as modificationName, @[prefix]pack.id as packId, @[prefix]pack.revision as packRevision
FROM @[prefix]pp
LEFT JOIN @[prefix]specification ON @[prefix]specification.ppid = @[prefix]pp.id
LEFT JOIN @[prefix]modifications ON @[prefix]modifications.id = @[prefix]specification.modificationId
LEFT JOIN @[prefix]pack ON @[prefix]pack.id = (SELECT MAX(@[prefix]pack.id) FROM @[prefix]pack WHERE @[prefix]pack.modificationId = @[prefix]specification.modificationId AND archived = 0)
WHERE @[prefix]pp.id IN(SELECT ppid FROM @[prefix]specification)');
$specificationsUP = array_merge($specificationsUP, $specificationsPP);
...
#engine::debug_printArray($lastSpecificationIds);
#$lastModificationIds = engine::DB()->getCol('SELECT id FROM @[prefix]modification t1 WHERE productId = (SELECT MAX(id) FROM @[prefix]modification t2 WHERE t2.productId = t1.productId)');
#engine::debug_printArray($specificationsPP);
// (
//     [id] => 21
...
<div class="card card-success">
    ...
    <style>
        ...
        .factory_schedule  tr.cat-7 td.cat{
            background-color: #313131;
            width: 5px;
        }

        tr.cat-4 td.cat a, tr.cat-5 td.cat a, tr.cat-6 td.cat a, tr.cat-7 td.cat a,
        tr.cat-4 td.cat, tr.cat-5 td.cat, tr.cat-6 td.cat, tr.cat-7 td.cat
        {
    ...
    <div class="card-body table-responsive p-0" style="overflow-y: hidden !important;">
        ...
        $GLOBALS['startMonth'] = (((int) date('m')) - 1);
        $GLOBALS['thisMonth'] = $GLOBALS['startMonth'];
        $GLOBALS['superCatStart'] = 4;
        $GLOBALS['superCat'] = $GLOBALS['superCatStart'];
        $GLOBALS['normativ'] = 0;
        $GLOBALS['normativPlus'] = 0;
        $GLOBALS['cat4maxDiff'] = -1;
        $GLOBALS['cat4minDiff'] = -1;

        function getCatAfter2($orderSum){
            if(($GLOBALS['thisMonth']==$GLOBALS['startMonth'] && ($GLOBALS['superMonth'][$GLOBALS['thisMonth']] + $orderSum) > $GLOBALS['limit']) || ($GLOBALS['thisMonth'] != $GLOBALS['startMonth'] && $GLOBALS['superMonth'][$GLOBALS['thisMonth']] > $GLOBALS['limit'])){
                $GLOBALS['thisMonth']++;
                $GLOBALS['superCat']++;
        ...
        function getCategory($days){
            if($days <= ((($GLOBALS['normativ']-$GLOBALS['normativPlus'])/3)*1)) return 3;
            if($days <= ((($GLOBALS['normativ']-$GLOBALS['normativPlus'])/3)*2)) return 2;
            return 1;
        }

        function getCategoryForCat4($days){
            if($days - $GLOBALS['cat4minDiff'] <= ((($GLOBALS['cat4maxDiff']-$GLOBALS['cat4minDiff'])/3)*1)) return 3;
            if($days - $GLOBALS['cat4minDiff'] <= ((($GLOBALS['cat4maxDiff']-$GLOBALS['cat4minDiff'])/3)*2)) return 2;
            return 1;
        }
        ...
        function getCategoryNew($days){
            if($days - $GLOBALS['firstFromCat4'] <= ((($GLOBALS['maxDiff'] - $GLOBALS['firstFromCat4'])/3)*1)) return 2;
            if($days - $GLOBALS['firstFromCat4'] <= ((($GLOBALS['maxDiff'] - $GLOBALS['firstFromCat4'])/3)*2)) return 1;
            return 0;
        }
        ...
        foreach($processingOrders as $k => $order){
            ...
            if($processingOrders[$k]['cat'] == 4 && $GLOBALS['cat4maxDiff'] < $processingOrders[$k]['diff']) $GLOBALS['cat4maxDiff'] = $processingOrders[$k]['diff'];
            if($processingOrders[$k]['cat'] == 4 && ($GLOBALS['cat4minDiff'] > $processingOrders[$k]['diff'] || $GLOBALS['cat4minDiff'] == -1)) $GLOBALS['cat4minDiff'] = $processingOrders[$k]['diff'];
        }

        foreach($processingOrders as $k => $order){
            if($order['cat'] == 4) $processingOrders[$k]['cat'] = getCategoryForCat4($order['diff']);
        }
        #engine::debug_printArray($GLOBALS['cat4maxDiff']);
        #engine::debug_printArray($GLOBALS['cat4minDiff']);
        $productIds = [];
        ...
        function getMFColor($text){
            switch(mb_strtolower($text)){
                case 'готов':
                    return 'color: #fff; background-color: #40c728; text-align: center;';
                    break;
                case 'запущен':
                    return 'color: #fff; background-color: #8034fa; text-align: center;';
                    break;
                case 'нет':
                    return 'color: #000; background-color: #ddd; text-align: center;';
                    break;
                case 'в работе':
                    return 'color: #fff; background-color:  #3486fa; text-align: center;';
                    break;
                case 'проблема':
                    return 'color: #fff; background-color:  #fba200; text-align: center;';
                    break;
                default:
                    return '';
            }
...
<span class="b startDate" data-tooltip="Дата создания заказа">'.date("Y-m-d", strtotime($order['moment'])).($order['cat'] > 0 ? '<br>'.@$GLOBALS['monthNames'][$order['superMonth']] : '').'</span>
</td>';
     ...
     <div class="dropdown-menu" role="menu" style="">
       <a class="dropdown-item" target="_blank" href="/index.php?component=moysklad&page=product&productId='.$order['productId'].'">Товар</a>
        '.$pdmAdd.$pdmContextAdd.'
            ...
            echo '<td class="sum dt-l" data-tooltipl="Сумма в работе нарастающим итогом в тысячах рублей"><span class="sum">'.number_format((int) ($sumNI / 100), 0, '', ' ').'₽</span><span class="subsum">'.number_format((int) ($s / 100), 0, '', ' ').'₽</span></td>';
...
$date2 = strtotime($nf_year.'-'.($nf_month).'-01');
...
$GLOBALS['estimated'] = (int)helper::datediff($ojsource);
